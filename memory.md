...
**Bloque destacado, publicado el 18/08/2026.** Arriba de la cinta va la cita de Verónica en grande, que **aparece línea por línea ligada a la posición del scroll** (`animation-timeline: view()`, un tramo distinto por línea), después la autoría, y después una franja de tres datos que se abre desde el centro. Es la única parte del sitio con animación *ligada* al scroll: todo el resto usa `.reveal`, que es animación *disparada* (entra, se ejecuta, terminó, siempre el mismo `translateY` de 34px). La cita está cortada en líneas a mano pero es **textual**, no se editó.

**La franja de datos son hechos del programa, no resultados de alumnas.** Dice "2 por semana / en casa o en el gimnasio", "Cada 2 semanas / con tus datos, no a ojo" y "Jimena, siempre / no hay un equipo detrás". Los tres ya estaban publicados en la sección Programa, así que son verificables. Se usó la estructura cifra + unidad + plazo del patrón 1 del benchmark 03 **sin inventar métricas**: Jimena todavía no tiene resultados propios de alumnas y la regla de no inventar sigue firme. El tercer dato además dice por primera vez en el sitio la ventaja de que atiende una sola persona, que el benchmark marcó como lo único que ninguno de los 18 sitios relevados puede copiar.
