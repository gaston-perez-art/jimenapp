...
> **Reemplazado el 27/08/2026, mismo día, por pedido de Jimena: el adicional pasó a ser su propio plan.** La franja al pie de la tarjeta única desapareció y ahora hay **dos tarjetas en paralelo** —"Método Raíz" y "Método Raíz + Tu entrenadora en vivo"— y en la segunda la clase en vivo **sí es el item 08**. La advertencia de arriba sigue valiendo pero cambió de lugar: prometer que está incluido es correcto en el plan 2, porque el plan 2 lo incluye; lo que no puede pasar es que el item 08 aparezca en la tarjeta del plan base.
>
> **Las dos listas están escritas enteras, no por referencia.** El plan 2 repite los siete items del plan base en vez de decir "todo lo anterior, más la clase en vivo": una lista de un item al lado de una de siete hace ver al plan más caro como el que trae menos, por más que la letra chica diga lo contrario. **El costo de esa decisión es que son dos copias del mismo texto: si se toca un feature, hay que tocarlo en las dos.** Está anotado en el HTML.
>
> **El plan 2 sigue sin precio publicado**, que es la parte de la decisión original que no cambió (docx, sección 6). En el lugar del monto va "Todo el programa, más yo del otro lado" y debajo **"¿Querés saber más?"** —Jimena cambió ese renglon el 27/08: antes decía "el valor te lo cuento en la entrevista", que nombraba el precio justo en la tarjeta que no lo publica—. También sacó la bajada debajo del título de la sección. **La consecuencia es que en pantalla ya no hay ni una línea que explique por qué falta el precio**, así que el único registro de que es deliberado son este párrafo y el comentario del HTML. No completar esa cifra: es una decisión, no un olvido.
>
> **Lo que esto abre:** el sitio vuelve a tener más de una opción a la vista, que es justo lo que se había cerrado el 13/08 al pasar de tres planes a uno. La diferencia es que acá los dos planes son el mismo programa —no tres productos distintos— y solo uno publica precio, así que la decisión de la visitante sigue siendo una sola. Vale medirlo igual: si aparecen consultas preguntando "¿cuál me conviene?", el plan 2 está compitiendo con el base en vez de sumarle.

**Verificado en `localhost:8899` el 27/08/2026** (dos tarjetas, 7 y 8 items, el precio argentino resuelto en la primera y sin monto en la segunda) y medido a **360, 768 y 1024px**: apilan por debajo de 960, no hay desborde horizontal a 360 y los dos CTA miden 52px de alto.

**Las dos franjas de precio se igualan con `subgrid`.** Medían distinto por contenido —309 contra 333px— y los dos CTA quedaban desfasados 23px, que a esa escala se ve. Con `grid-template-rows:subgrid` en las tarjetas la fila de la franja es la misma para las dos, así que se igualan solas y van a seguir igualándose si cambia el copy. Donde no haya `subgrid` la regla se ignora y vuelve el desfasaje: degrada a feo, no a roto. Verificado: 319 y 319px, diferencia de 0 entre los CTA a 1280 y a 1024.
...
### 23/09/2026 — Monograma "J.", el proceso con maquetas y los planes sin la lista repetida

Tres cosas que Gastón marcó como feas, y las tres se cambiaron el mismo día.

**1. El header deja de llevar la raíz y el favicon pasa a ser "J.".** A 30px la marca sólida se leía como un árbol invertido y nadie la asociaba con entrenamiento. El header queda con el nombre solo, "Jimena Ibañez." con el punto bronce. El favicon es una J blanca en Archivo 800 con el punto bronce, sobre un cuadrado vino redondeado: `docs/img/marca/monograma-16/32/48/512.png` y `monograma-apple-touch.png` (180, sin redondeo, porque iOS aplica su propia máscara). El master de 1024 está en `materiales/marca/`. Se descartó el emoji 🏋️‍♀️ porque se dibuja distinto en cada sistema y el de Apple es una mujer rubia genérica. Esta vez sí salió bien hecho a máquina, y confirma la regla del 23/08: con letras, el render local funciona; lo que falla es *dibujar*. **La raíz sigue siendo la marca del método**, en el og:image y en la firma del footer. Los archivos viejos `marca-*` y `favicon-*` quedan en el repo sin uso.

**2. "El proceso, paso a paso" cambia las cuatro ilustraciones por maquetas con texto real.** Las ilustraciones eran abstractas: burbujas sin texto, un "41%" sin decir de qué y barras sin etiqueta. Además, en mobile no se veían, porque el panel fijo se ocultaba debajo de 960px. Ahora cada etapa trae texto y una maqueta, con un solo marcado para escritorio y mobile, y la maqueta arranca cuando la etapa entra (`.in-view`). Hubo una segunda vuelta el mismo día:
- **01 · chat en loop** (16s). Aparecen los tres puntitos de "escribiendo" en el lugar de cada mensaje, entra la burbuja, y al terminar la charla se desvanece y vuelve a empezar. Cada mensaje tiene su lugar reservado, así que la tarjeta no salta.
- **02 · ficha "Tu punto de partida"**. Se anima una sola vez y queda quieta. No lleva barras: una barra de "grasa 66%" no dice nada.
- **03 · "Tu plan, armado con lo que me contaste"** (loop de 12s). Cada frase de la alumna ("Los viernes no llego al gimnasio") tiene al lado la decisión del plan que sale de ella ("El viernes entrenás en casa, con mancuernas"). Reemplazó a una semana tipo calendario que, según Gastón, "parece genérica": una semana es una plantilla con otro nombre.
- **04 · gráfico vivo** (loop de 10s). La curva de la sentadilla mes a mes se dibuja, se estanca en una meseta, aparece el AJUSTE en el mes 3 y la curva vuelve a subir hasta 35 kg. El trazo se anima en forma lineal a propósito: la meseta y el ajuste están sincronizados con el largo del trazo.

Las cifras dicen EJEMPLO y no son de ninguna alumna. Sin JS o con movimiento reducido, todo se ve completo y quieto. Se sacaron el panel fijo, los SVG `pv*` y los dos scripts.

**3. Los planes: dos cards lado a lado, también en mobile, con la diferencia adentro.** Cada tarjeta repetía los 7 ítems con su descripción, y la única diferencia, el 08, quedaba al pie de la segunda: dos columnas de ~1200px que parecían la misma. Una primera vuelta sacó la lista afuera de las tarjetas y Gastón la rechazó, porque así las dos seguían pareciendo iguales. Quedó así: en el **primer renglón** de cada tarjeta, en el mismo lugar, "Clases en vivo 1 a 1". En una va en un bloque vino con check; en la otra, apagado y tachado. Debajo, los 7 ítems **solo con títulos** y, al final, el CTA. Los renglones de las dos tarjetas se alinean por `subgrid`. Las descripciones largas quedan guardadas en el comentario HTML de `#servicios`. La sección pasó de ~2400px a 725px en escritorio. El riesgo del 27/08 ("una lista de un ítem al lado de una de siete hace ver al plan más caro como el que trae menos") no aplica: las dos tarjetas muestran la lista completa. **Sigue sin precio publicado**, y el `data-ga="whatsapp_precio"` se conservó.

**3b. Feedback de Jimena: el recomendado es el Método Raíz.** El objetivo del negocio es vender el Método Raíz, porque a Jimena no le da el tiempo para dar muchas clases en vivo. Ese plan lleva el badge "Recomendado" y el borde destacado, y en mobile arranca elegido. **No muestra lo que no incluye**: se sacó el renglón tachado "Clases en vivo · No incluidas". La clase en vivo aparece solo en la otra tarjeta, arriba y en positivo. Sin ese renglón, la lista del recomendado arranca pegada a la cabecera, y los dos botones siguen alineados.

**3c. "Método Raíz" se dice una sola vez en la sección.** Aparecía cinco veces en un solo escaneo: en el título, en los dos tiles y en las dos tarjetas. Queda solo en el título ("Dos formas de hacer el Método Raíz"), y los planes se nombran por lo que los distingue: **"El programa"** y **"El programa + en vivo"**. Se sacaron las etiquetas mono de arriba del nombre, que repetían lo mismo. En los tiles de mobile, el texto va centrado en vertical.

**3d. El recomendado tiene su propio bloque: "A tu ritmo".** Sin un bloque en el renglón de la diferencia, la tarjeta del recomendado quedaba con un hueco abajo y las dos se veían desbalanceadas en escritorio. Ahora cada una lleva su ventaja en el mismo lugar y con el mismo peso. En "El programa": **"A tu ritmo · Entrenás cuando puedas, con mi seguimiento"**. En la otra: "Clases en vivo 1 a 1". Es la contracara positiva de no tener clases, y le habla a la mujer que no tiene tiempo fijo, que es el público de `contexto.md`.

**4. En mobile, los planes se eligen a la Netflix.** Las dos tarjetas completas lado a lado a 360px quedaban de ~150px, con cada ítem partido en dos o tres renglones, y a Gastón le parecieron feas. Ahora, a 720px o menos, arriba hay dos tiles lado a lado para elegir: "Método Raíz · Sin clases en vivo" y "Método Raíz + en vivo · Con clases 1 a 1". El elegido se pinta con el degradé vino→bronce y un check. Debajo va una sola tarjeta con el detalle del plan elegido. Son dos radios y `:checked`, sin JS. Arranca elegido el Método Raíz, que es el recomendado (ver 3b). Ojo: en mobile, la tarjeta oculta no pasa por el `.reveal`, porque el observador nunca la ve intersectar y quedaría en opacity 0 al elegirla. Entra con su propia animación.

**5. El menú hamburguesa estaba roto a la vista.** `.navlinks a` (0,1,1) le pisaba el padding a `.navcta` (0,1,0), y "Escribime" quedaba como una píldora apretada contra el texto. Ahora los links son renglones con separador, el CTA va a lo ancho y la hamburguesa se vuelve una X al abrir.

**6. CTA de WhatsApp fijo abajo, solo en mobile.** Lo propuso Gastón. En mobile el CTA del hero desaparece al primer scroll y después hay cuatro o cinco pantallas sin un botón a mano. La barra aparece cuando el botón del hero sale de pantalla y se oculta sobre Planes, Contacto y el footer, que tienen sus propios botones (y ahí taparía los links legales). También se oculta con el menú abierto. Lleva el mismo texto y destino que el hero ("Quiero mi entrevista gratuita"), no "consultar precio": la entrevista gratuita es la puerta de entrada. **Mide aparte con `data-ga="whatsapp_sticky"`**, para ver en GA4 si suma conversiones o solo reparte los clics del hero. Sin JS no aparece.
...
**Cómo trabajo:** hasta el 23/09/2026 era un panel fijo con `position: sticky`, al estilo de Equinox. Ahora son cuatro etapas unidas por una línea vertical, cada una con su texto y una maqueta con contenido real (chat, ficha, plan, gráfico), y se ven igual en escritorio y en mobile. Detalle en la entrada del 23/09/2026.
