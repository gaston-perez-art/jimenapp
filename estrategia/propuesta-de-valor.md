# Propuesta de valor

> **Estado: posicionamiento decidido.** Abierto el 11/08/2026, en discusión entre Jimena y Gastón. Jimena resolvió la decisión abierta el 12/08/2026 (ver sección correspondiente más abajo). Lo que queda pendiente de este documento es lo que lista la sección "Qué falta".

## Lo que ofrecemos hoy

Programas personalizados de fuerza y nutrición para mujeres de más de 30, mamás y profesionales con poco tiempo. Rutinas adaptables a casa o gimnasio desde 2 días por semana, sin dietas restrictivas, con seguimiento mensual real y evaluación inicial en vez de plantillas.

Está bien construida. Tiene un público claro, evita las promesas mágicas y describe con honestidad lo que se entrega.

## El problema de esa propuesta

Es buena y es genérica al mismo tiempo.

Esa descripción la puede firmar cualquier entrenadora online de Buenos Aires. "Personalizado", "sin dietas restrictivas" y "para mujeres ocupadas" son hoy la categoría entera, no un diferencial. Compitiendo ahí el precio lo fija el mercado, y la única variable que queda es cuánta gente ve el Instagram. Es una carrera que se gana con volumen de contenido, no con calidad de servicio.

## El diferencial que sí tenemos

Está escrito en `contexto.md` y está enterrado como cuarto punto de una lista: **salud hormonal femenina aplicada al entrenamiento y la nutrición.** Perimenopausia, menopausia, resistencia a la insulina, SOP e hipotiroidismo. En la web aparece como un chip entre siete.

Eso sí es defendible, por tres razones:

• **El segmento está mal atendido.** Una mujer de 45 años en perimenopausia que sigue una rutina genérica no obtiene resultados, y suele haber pasado ya por dos o tres entrenadores que no entendieron por qué.
• **La disposición a pagar es más alta.** No se compra estética. Se compra resolver un problema que ya se intentó resolver sin éxito y que afecta el descanso, la energía y el ánimo.
• **Es difícil de copiar.** Requiere formación específica. "Entreno mujeres" lo dice cualquiera mañana. "Trabajo con mujeres en perimenopausia y sé cuándo derivarte a un endocrinólogo" no.

## La decisión tomada

**¿A quién le hablamos: mujeres +30 en general, o especialización hormonal?**

Era la decisión que bloqueaba a las demás, porque define el precio, el canal y todo el mensaje de la web. **Jimena la resolvió el 12/08/2026: especialización hormonal.** El público es mujeres +35 que buscan recomposición corporal (pérdida de grasa + ganancia de masa muscular) teniendo en cuenta sus cambios hormonales — resistencia a la insulina, SOP, hipotiroidismo, perimenopausia o menopausia.

Las dos opciones eran legítimas. La general captaba más volumen y competía por precio. La especializada capta menos gente, la convierte mejor y sostiene otro valor — que es la apuesta que se hizo, alineada con el diferencial descrito arriba. La contradicción que delataba la indecisión (footer en +40, resto del sitio en +30) ya está resuelta: todo el sitio dice +35.

**Nota importante:** Jimena recién empieza — no tiene todavía alumnas ni resultados propios documentados. La web no debe afirmar experiencia previa ni resultados conseguidos hasta que existan de verdad; el mensaje se apoya en el método (especialización, evaluación real, cambios hormonales considerados), no en un historial que todavía no existe.

## Qué falta

Este documento cubre la propuesta de valor. Todavía no existen, y conviene armarlos en este orden una vez resuelto el posicionamiento:

• Misión, visión y propósito
• Business Model Canvas
• FODA con lectura de la competencia directa

## Análisis económico

El business case está en [`business-case.md`](business-case.md), en esta misma carpeta: rentabilidad por hora, techo del negocio y la secuencia recomendada para moverse. **Nota:** ese análisis todavía está escrito sobre la estructura vieja de tres planes, y ya quedó dos veces atrás — el 13/08/2026 se pasó a un solo programa a USD 35/mes, y el 27/08/2026 ese precio único fue reemplazado por la estrategia de tres etapas y dos escalas de [`estrategia-de-precios-metodo-raiz.docx`](estrategia-de-precios-metodo-raiz.docx). **El precio vigente es USD 45/mes para Argentina y USD 90/mes internacional, etapa fundadoras, con vigencia desde el 1/09/2026.** Pendiente de reflejar en el business case.

Los dos documentos se leen juntos. El business case muestra que el precio es la palanca más fuerte del negocio, y este documento explica qué hace falta para poder subirlo.
